...
const vacancyController = {
    getAll: async (req, res) => {
        try {
            ...
            let query = `
                SELECT v.id, v.title, v.description, v.salary_min as salaryMin, v.salary_max as salaryMax,
                       v.status, d.id as departmentId, d.name as departmentName,
    ...
    getById: async (req, res) => {
        try {
            ...
            const [rows] = await pool.query(`
                SELECT v.id, v.title, v.description, v.salary_min as salaryMin, v.salary_max as salaryMax,
                       v.status, v.department_id as departmentId, d.name as departmentName,
    ...
    create: async (req, res) => {
        try {
            ...
            if (!title) return res.status(400).json({ error: 'Vacancy title is required' });
            const [result] = await pool.query(`
                INSERT INTO vacancies_hr (title, description, salary_min, salary_max, department_id, status, created_at)
                VALUES (?, ?, ?, ?, ?, ?, NOW())
            `, [title, description || null, salaryMin || null, salaryMax || null, departmentId || null, status || 'open']);
            res.status(201).json({ message: 'Vacancy created successfully', id: result.insertId });
        } catch (error) {
            console.error(error);
            res.status(500).json({ error: error.message });
        }
        ...
        try {
            ...
            const [result] = await pool.query(`
                ...
                SET title = COALESCE(?, title), description = COALESCE(?, description),
                    ...
                    department_id = COALESCE(?, department_id), status = COALESCE(?, status), updated_at = NOW()
                WHERE id = ?
    ...
    close: async (req, res) => {
        try {
            const { id } = req.params;
            const [result] = await pool.query(`UPDATE vacancies_hr SET status = 'closed', closed_at = NOW(), updated_at = NOW() WHERE id = ? AND status != 'closed'`, [id]);
            if (result.affectedRows === 0) return res.status(404).json({ error: 'Vacancy not found or already closed' });
